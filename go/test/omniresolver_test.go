...
import (
	...
	omni "github.com/voxgig-sdk/dorognoe-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.DorognoeError); is {
				out := map[string]any{
					"name":    "DorognoeError",
...
func omniCtx(arg any, client *sdk.DorognoeSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.DorognoeSDK); is {
			client = live
