...
class TestCityEntity:
    ...
    def test_should_stream(self):
        ...
        base = DorognoeSDK.test(seed, None)
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set DOROGNOE_TEST_CITY_ENTID JSON to run live")
        client = setup["client"]
...
def _city_basic_setup(extra):
    ...
    client = DorognoeSDK.test(options, extra)
    ...
    idmap_resolved = helpers.to_map(
        env.get("DOROGNOE_TEST_CITY_ENTID"))
    ...
    if env.get("DOROGNOE_TEST_LIVE") == "TRUE":
        ...
        client = DorognoeSDK(helpers.to_map(merged_opts))

    _live = env.get("DOROGNOE_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("DOROGNOE_TEST_EXPLAIN") == "TRUE",
        "live": _live,
