# Dorognoe SDK test runner

from __future__ import annotations
import os
import json

from dorognoe_sdk.utility.voxgig_struct import voxgig_struct as vs


class DorognoeTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            DorognoeTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = DorognoeTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = DorognoeTestRunner.getenv("DOROGNOE_TEST_LIVE")
        override = DorognoeTestRunner.getenv("DOROGNOE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = DorognoeTestRunner.getenv(key)
        ...
        explain = DorognoeTestRunner.getenv("DOROGNOE_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["DOROGNOE_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if DorognoeTestRunner._test_control is not None:
            return DorognoeTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                DorognoeTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            DorognoeTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = DorognoeTestRunner.load_test_control()
...
def load_env_local():
    DorognoeTestRunner.load_env_local()


def env_override(m):
    return DorognoeTestRunner.env_override(m)


def entity_data(v):
    return DorognoeTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return DorognoeTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return DorognoeTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return DorognoeTestRunner.load_test_control()


def live_client_options():
    return DorognoeTestRunner.live_client_options()


def live_delay_ms():
    return DorognoeTestRunner.live_delay_ms()
