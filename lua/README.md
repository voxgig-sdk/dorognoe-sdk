...
The Lua SDK for the Dorognoe API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:City()` — each with the same small set of operations (`list`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/dorognoe-sdk/releases)),
...
```
lua/
├── dorognoe_sdk.lua    -- Main SDK module
...
The main module (`dorognoe_sdk`) exports the SDK constructor
