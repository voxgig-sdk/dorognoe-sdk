...
[MCP](https://modelcontextprotocol.io) server exposing the Dorognoe SDK as
two agent tools — `dorognoe_list` and `dorognoe_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/dorognoe-mcp)
make build

# 2. Provide credentials via the environment
export DOROGNOE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user dorognoe \
  -- /absolute/path/to/dorognoe-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./dorognoe-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// dorognoe_list: first page of records
{ "entity": "city" }
{ "entity": "city", "query": { } }

// dorognoe_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/dorognoe-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user dorognoe \
     -- "$PWD"/dist/*/dorognoe-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `dorognoe_list` and `dorognoe_load` tools now appear
   in new sessions. Ask the agent to *"list city using dorognoe"*
   and it calls `dorognoe_list` with `{"entity":"city"}`.
...
```sh
export DOROGNOE_APIKEY=sk_live_xxx            # API key
export DOROGNOE_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `dorognoe_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `dorognoe_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `DOROGNOE_APIKEY` | API key sent with every request. |
| `DOROGNOE_BASE` | Optional override of the API base URL. |
...
```sh
./dorognoe-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"dorognoe_load","arguments":{"entity":"city","query":{"id":1}}}}'
```
