...
MCP server exposing the Dorognoe SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `dorognoe_list` | `entity`, optional `query` map | First page of records as JSON |
| `dorognoe_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `dorognoe_list` and
`dorognoe_load` tools become available in new sessions.
