...
module DorognoeUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = DorognoeContext.new({}, nil)
    end
    op = ctx.op || DorognoeOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || DorognoeResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(DorognoeError) ? err.msg : err.to_s
    ...
    sdk_err = DorognoeError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(DorognoeError)
