...
//     (\`new DorognoeSDK(...)\` / \`DorognoeSDK.test(...)\` become
//     \`DorognoeSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"city":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new DorognoeSDK(...)\` and any
// \`DorognoeSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+DorognoeSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/DorognoeSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
