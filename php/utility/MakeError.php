...
// Dorognoe SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class DorognoeMakeError
{
    public static function call(?DorognoeContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new DorognoeContext([], null);
        }
        $op = $ctx->op ?? new DorognoeOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new DorognoeResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof DorognoeError) ? $err->msg : (string)$err;
        $msg = "DorognoeSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new DorognoeError('', $msg, $ctx);
        ...
        if ($err instanceof DorognoeError) {
