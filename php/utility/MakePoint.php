...
// Dorognoe SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class DorognoeMakePoint
{
    public static function call(DorognoeContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof DorognoeError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $select_def = DorognoeHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = DorognoeHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
