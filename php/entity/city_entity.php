...
class CityEntity
{
    ...
    {
        if ($args) {
            $this->_data = DorognoeHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = DorognoeHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   the SDK boundary; throws DorognoeError on failure (item-5 convention).
     */
