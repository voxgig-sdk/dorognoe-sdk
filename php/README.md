...
The PHP SDK for the Dorognoe API — an entity-oriented client using PHP conventions.
...
print_r(array_map(fn($item) => $item->data_get(), $city));
```
...
```php
require_once 'dorognoe_sdk.php';
$client = new DorognoeSDK($options);
```
...
```
php/
├── dorognoe_sdk.php          -- Main SDK class
...
The main class (`dorognoe_sdk.php`) exports the SDK class
