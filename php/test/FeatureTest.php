...
class FtCtrl extends DorognoeControl
{
...
class FtClient
{
    ...
    private DorognoeUtility $utility;

    public function __construct(DorognoeUtility $utility, array $options)
    {
    ...
    public function get_utility(): DorognoeUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (DorognoeContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public DorognoeContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (DorognoeContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = DorognoeConfig::make_feature($name);
    ...
    private static function buildUrl(DorognoeSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof DorognoeError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new DorognoeResponse($fetched);
        }
    ...
    private function fail(DorognoeContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (DorognoeContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
