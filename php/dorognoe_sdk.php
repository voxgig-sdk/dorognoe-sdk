...
class DorognoeSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = DorognoeHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = DorognoeHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!DorognoeFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, DorognoeFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return DorognoeUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = DorognoeHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = DorognoeHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = DorognoeHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new DorognoeError($op . "_allow",
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = DorognoeHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new DorognoeError("graphql_error",
                "DorognoeSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
