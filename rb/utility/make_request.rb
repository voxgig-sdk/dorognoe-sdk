...
module DorognoeUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = DorognoeResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = DorognoeResponse.new(fetched)
    else
