...
class CityEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = DorognoeSDK.test(seed, nil)
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set DOROGNOE_TEST_CITY_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def city_basic_setup(extra)
  ...
  entid_env_raw = ENV["DOROGNOE_TEST_CITY_ENTID"]
  ...
  if env["DOROGNOE_TEST_LIVE"] == "TRUE"
    ...
    client = DorognoeSDK.new(Helpers.to_map(merged_opts))
  end
