...
class CityEntity
  ...
  def data_set(args)
    if args
      @_data = DorognoeHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = DorognoeHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Array<City>, Array] the matching City items; raises DorognoeError on failure
