...
require_relative 'Dorognoe_types'
...
class DorognoeSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = DorognoeHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = DorognoeHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, DorognoeFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = DorognoeHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = DorognoeHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = DorognoeHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => DorognoeError.new(
        "#{op}_allow",
        "DorognoeSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = DorognoeHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = DorognoeError.new(
        "graphql_error", "DorognoeSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
