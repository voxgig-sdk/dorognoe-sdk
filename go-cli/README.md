...
boru-driven command-line client **and** interactive REPL for the Dorognoe
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/dorognoe-cli)
make build
...
DOROGNOE_BASE=https://api.example.com ./dorognoe-cli list city
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/dorognoe-cli
   ```
...
Run with no arguments to open a REPL (prompt `dorognoe>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/dorognoe-cli`. |
...
(`./dorognoe-cli <expr>`) and interactively (the REPL), and expressions compose the
